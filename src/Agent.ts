...
class Agent {
  ...
  public rotate(dr: number): void {
    this.position.rotation =
      (this.position.rotation + dr + 2 * Math.PI) % (2 * Math.PI); // Keep rotation within [0, 2π]
  ...
  public render() {
    if (AgentConfigData.RenderSensor) {
      this.Sensor.render(canvasVariables.context);
    }

    // Render the agent
    canvasVariables.context.beginPath();

    // Move to the first vertex
    canvasVariables.context.moveTo(this.polygon[0].x, this.polygon[0].y);

    // Draw lines to subsequent vertices
    for (let i = 1; i < this.polygon.length; i++) {
      canvasVariables.context.lineTo(this.polygon[i].x, this.polygon[i].y);
    }

    // Close the polygon
    canvasVariables.context.closePath();

    // Fill and stroke the polygon
    canvasVariables.context.fillStyle = AgentConfigData.Rendering.FillColor;
    canvasVariables.context.fill();
    canvasVariables.context.strokeStyle = Utilities.lightenHexColor(
      AgentConfigData.Rendering.FillColor,
      1
    );
    canvasVariables.context.lineWidth = AgentConfigData.Rendering.StrokeWidth;
    canvasVariables.context.stroke();
  }
...
class Food {
  ...
  public render() {
    ...
    canvasVariables.context.fillStyle = AgentConfigData.FoodSettings.FoodColor;
    canvasVariables.context.fill();
    canvasVariables.context.strokeStyle = Utilities.lightenHexColor(
      AgentConfigData.FoodSettings.FoodColor,
      1
    );
    canvasVariables.context.lineWidth = AgentConfigData.Rendering.StrokeWidth;
    canvasVariables.context.stroke();
